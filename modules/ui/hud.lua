...
function hud.drawEntry()
    if hud.entryVisible then
        ...
        utils.createInteractionHub("进入地铁站", "UI_Apply", true)
        hud.interactionHUDEntry = true
    else
        if hud.interactionHUDEntry then
            utils.createInteractionHub("进入地铁站", "UI_Apply", false)
        end
...
function hud.drawExit()
    if hud.exitVisible then
        ...
        utils.createInteractionHub("离开地铁站", "UI_Apply", true)
        hud.interactionHUDExit = true
    else
        if hud.interactionHUDExit then
            utils.createInteractionHub("离开地铁站", "UI_Apply", false)
        end
...
function hud.drawDoor()
    if hud.doorVisible then
        ...
        utils.createInteractionHub("使用门", "UI_Apply", true) -- Spam it to make sure it really gets diplayed and not gets interrupted
        hud.interactionHUDDoor = true
    else
        if hud.interactionHUDDoor then
            utils.createInteractionHub("使用门", "UI_Apply", false)
        end
...
function hud.drawTrain()
    if hud.trainVisible then
        ...
        utils.createInteractionHub("进入地铁", "UI_Apply", true)
        hud.interactionHUDTrain = true
    else
        if hud.interactionHUDTrain then
            utils.createInteractionHub("进入地铁", "UI_Apply", false)
        end
